# Apply the trained model to OpenCV
...
# Crop, convert to grayscale, blur and resize:
    ...
    pred_probab, pred_class = keras_predict(loaded_model, ar)
...
# Now we have to predict the alphabet from the input image. 
# Our model will give outputs as integers rather than alphabets that’s because the labels are given as integers (1 for A, 2 for B, 3 for C and so on..)

# Pass the input image into the classifier:
#model is our classifier and image is input image we are passing
#----------------------------
#def keras_predict(loaded_model, image):
#    data = np.asarray(image, dtype="int32")

#    pred_probab = loaded_model.predict(data)[0]
